#!/usr/bin/env bash
#
# Ubuntu / Debian - zRAM + Swap Setup (ext4, xfs, and Btrfs aware)
#
# Features:
#   - 12 GiB (12288 MiB) zRAM (zstd, priority 100) via systemd-zram-generator
#   - 32 GiB swapfile (priority 10)
#   - Distro + package manager detection (Ubuntu/Debian family with apt-get)
...
#   - Lazy metadata refresh (only updates apt cache if packages are missing)
...
detect_distro() {
    ...
    if [[ "$DISTRO_FAMILY" == "unsupported" ]]; then
        ...
        echo "This script is purely for Ubuntu/Debian-family distros (apt)."
        exit 1
    fi
}
...
detect_pkg_manager() {
    ...
    command -v apt-get >/dev/null 2>&1 || {
        echo "ERROR: apt-get not found despite a Debian-family os-release."
        exit 1
    }
    PKG_MANAGER="apt-get"
...
# Managed by zram_swap_generator.sh
[zram0]
...
Ubuntu/Debian zRAM + Btrfs/ext4 Swapfile Setup Script
